 ...
 */
function video_match_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on video-match, use a find and replace
		* to change 'video-match' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'video-match', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'video-match' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'video_match_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'video_match_setup' );
 ...
 */
function video_match_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'video_match_content_width', 640 );
}
add_action( 'after_setup_theme', 'video_match_content_width', 0 );
 ...
 */
function video_match_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'video-match' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'video-match' ),
...
add_action( 'widgets_init', 'video_match_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function video_match_scripts() {
...
add_action( 'wp_enqueue_scripts', 'video_match_scripts' );
